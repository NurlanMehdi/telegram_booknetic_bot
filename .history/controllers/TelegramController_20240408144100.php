    ...
    public function handleIncomingMessages()
    {
        ...
        if (isset($updates["result"])) {
            foreach ($updates["result"] as $update) {
                ...
                $checkMessage = $this->messagesModel->getMessageById($message['message_id']);

                if(!$checkMessage){
                    $this->messagesModel->addMessage($chatId, $messageId, $updateId, $messageText, $username);
                }
        ...
        }elseif($services->getServiceById($messageText))
        {
            ...
            if($dateTime)
            {
                ...
                if(!empty($data['data']['dates'][$dateTime->format('Y-m-d')])){
                    ...
                    foreach($date as $val)
                    {
                        $timeRange .= "{$val['start_time']} - {$val['end_time']} \n";
                    }
                    $telegramHelper->sendMessage($chatId, "Sizə uyğun saat aralığını seçin. \n{$timeRange}",$this->apiEndpoint);
                }else{
                    $timeRange = '';
