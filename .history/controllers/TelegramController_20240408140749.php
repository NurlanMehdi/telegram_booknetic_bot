            ...
            if($dateTime)
            {
                ...
                if(!empty($data['data']['dates'][$dateTime->format('Y-m-d')])){
                   ...
                   // var_dump($data['data']['dates']);
                    ...
                    foreach ($array as $date => $events) {
                        // Her bir tarih için etkinlikleri işleyen döngü
                        $times = [];
                        foreach ($events as $event) {
                            // Etkinlik bilgilerini al
                            $start_time_format = $event['start_time_format'];
                            $end_time_format = $event['end_time_format'];
                            
                            // Saat aralığını oluştur
                            $timeRange = "$start_time_format - $end_time_format";
                            
                            // Zaten eklenmemişse, saat aralığını ekle
                            if (!in_array($timeRange, $times)) {
                                $times[] = $timeRange;
                            }
                        }
                    
                        // İlgili tarih ve birleştirilmiş saat aralıklarını $aviableDatesMessage değişkenine ekle
                        $aviableDatesMessage .= "<b>$date</b>: " . implode(', ', $times) . "\n";
                    }
                    
                   // $telegramHelper->sendMessage($chatId, "Seçdiyiniz günə uyğun nəticə tapılmadı. Zəhmət olmasa yeni tarix seçin. \n{$aviableDatesMessage}",$this->apiEndpoint);
          
                    echo $aviableDatesMessage;
