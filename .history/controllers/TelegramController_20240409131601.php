...
class TelegramController
{
        ...
        if (isset($updates["result"])) {
            foreach ($updates["result"] as $update) {
                if (isset($update["message"])) {
                    ...
                    $getMessageData = $this->messagesModel->getMessageData(
                        $chatId
                    );
                    $userInfo = explode(
                        ",",
                        $getMessageData["info"]["message"]
                    );
    ...
    private function processMessage($chatId, $username, $messageText)
    {
        ...
        if ($messageText == "/start") {
            ...
            $getMessageData = $this->messagesModel->getMessageData($chatId);
            $serviceData = $services->getServiceById(
                $getMessageData["service"]["message"]
            );

            $servicesInfo =
                "1: <b>{$serviceData["title"]}</b> - " .
                $serviceData["price"] .
