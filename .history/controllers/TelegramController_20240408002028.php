...
class TelegramController
{
    ...
    public function handleIncomingMessages()
    {
        $response = file_get_contents(
            $this->apiEndpoint . "getUpdates?offset=<872528163>"
        );
        ...
        if (isset($updates["result"])) {
            foreach ($updates["result"] as $update) {
                ...
                $checkMessage = $messages->getServiceById($message['message_id']);
                if(!$checkMessage){
                    $services->addMessage($message['chat']['id'], $message['message_id'], $update['update_id'],$messageText,$username);
                }
